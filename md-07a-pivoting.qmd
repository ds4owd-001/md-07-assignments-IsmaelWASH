...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    It's a package that brings together a collection of extensions designed to work together, allowing you to import/export data, manipulate variables and data tables, visualize data, and more.
...
levels_income_cat <- c("high income","low income","lower-middle income","upper-middle income")

waste_data_fct <- waste_data |> mutate(income_cat=factor(income_cat,levels=levels_income_cat))
```
...
```{r}
waste_data_long <- waste_data_fct |> pivot_longer(
  cols = food_organic_waste:yard_garden_green_waste,
...
```{r}
waste_data_long_mean <- waste_data_long |> group_by(income_cat,waste_category) |> 
...
No, I couldn't publish the table in this format because for the variables income_cat and waste_category we have a redundancy in the observations.
...
```{r}
waste_data_wider_mean <- waste_data_long_mean |> 
  pivot_wider(names_from = waste_category,
              values_from = mean_percent)
```

Yes, I could publish the table in this format because for each variable we have unique observations.
...
ggplot(waste_data_long_mean,mapping = aes(
  x= mean_percent,
  y= waste_category,
  fill= income_cat))+
  geom_col(position = position_dodge())+
  ggtitle("Waste Composition",subtitle = "Mean percentages of nine waste categories displayed by income categories")
